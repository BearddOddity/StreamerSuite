...
export default function AddChannelModal({ isOpen, onClose, onAdd }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-float-backdrop"
      ...
      <div className="relative w-[420px] bg-[#0a0a0f]/95 border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/50 backdrop-blur-xl animate-float-card-in overflow-hidden">
        <div className="px-5 pt-5 pb-4 flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center">
            <span className="text-[var(--accent-system)] text-base">+</span>
          </div>
          <div>
        ...
        <form onSubmit={handleSubmit} className="px-5 pb-5 space-y-4">
              ...
              {platforms.map((p) => (
                <button key={p.id} type="button" onClick={() => setPlatform(p.id)}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-sm font-medium border transition-all ${
                    platform === p.id ? p.activeColor : "border-white/[0.06] text-white/35 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/[0.1]"
                  }`}>
          ...
          <div>
            ...
            <input type="text" value={channelId} onChange={(e) => setChannelId(e.target.value)}
              ...
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-sm text-white/80 placeholder:text-white/20 focus:outline-none focus:border-[var(--accent-system)]/40" autoFocus />
          </div>
          <div>
            ...
            <input type="text" value={channelName} onChange={(e) => setChannelName(e.target.value)}
              placeholder="e.g. xQc"
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-sm text-white/80 placeholder:text-white/20 focus:outline-none focus:border-[var(--accent-system)]/40" />
          </div>
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={!channelId.trim() || !channelName.trim()}
