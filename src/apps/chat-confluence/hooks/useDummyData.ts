...
const twitchMessages = [
  "LET'S GOOO 🔥🔥🔥", "PogChamp PogChamp PogChamp", "KEKW that was hilarious",
  "monkaS monkaS monkaS", "Can we get some hype in chat??", "GG well played", "LULW",
  "Sadge", "widepeepoHappy", "This stream is so good", "First time here, love the content!",
  "HeyGuys", "VoHiYo", "Can you play my song request?", "modCheck", "catJAM", "pepeLaugh",
  "OMEGALUL", "EZ Clap", "Pog", "FeelsStrongMan", "Copium", "Based",
  "No way that just happened 💀", "Chat is moving so fast I can't keep up", "HYPERS",
  "SHEEEESH", "bruh moment", "this is so cursed", "W stream", "L take",
];

const kickMessages = [
  "LET'S GOOO", "This is fire 🔥", "First time on Kick, love it here",
  "Better than Twitch ngl", "GG!", "Hype hype hype!", "Can someone clip that?",
  "LFG!!!!", "This chat is so chill", "Just subbed! 🎉", "Who else is watching at 3am?",
  "POGGERS", "KEKW", "monkaW", "EZ", "W content", "This streamer is underrated",
  "Chat let's gooooo", "Can we get 100 viewers?", "Dropped a follow, great stream!",
];

const joystickMessages = [
  "Nice play!", "How did you do that combo?", "This game is so good", "GG WP",
  "Let's gooo", "That was insane", "Can you try a no-hit run?", "First time watching, this is awesome",
  "The vibes here are great", "Who else loves retro games?", "Speedrun any% when?",
  "This boss is brutal", "Clutch!!!", "No way 🤯", "Great stream as always",
];

const userPool: Record<Platform, ChatUser[]> = { twitch: twitchUsers, kick: kickUsers, joystick: joystickUsers };
const messagePool: Record<Platform, string[]> = { twitch: twitchMessages, kick: kickMessages, joystick: joystickMessages };
...
export function useDummyData() {
  ...
  const spawnMessage = useCallback(() => {
    ...
    const msg: ChatMessage = {
      ...
      content, timestamp: Date.now(), isDeleted: false,
    };
    setMessages((prev) => {
      const next = [...prev, msg];
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      seed.push({
        id: generateId(), platform,
        user: { ...user, id: `seed-${i}` },
        content, timestamp: Date.now() - (15 - i) * 2000, isDeleted: false,
      });
    }
    setMessages(seed);
    intervalRef.current = setInterval(() => {
      spawnMessage();
      if (Math.random() > 0.7) setTimeout(spawnMessage, 300);
      if (Math.random() > 0.9) setTimeout(spawnMessage, 600);
    }, 1200);
    return () => { if (intervalRef.current) clearInterval(intervalRef.current); };
  }, [spawnMessage]);

  return {
    messages, channels, activeChannel, setActiveChannel, isMultiChat, setIsMultiChat,
    clearMessages: () => setMessages([]),
    connectChannel: () => {}, disconnectChannel: () => {}, sendMessage: () => {},
    isConnecting: false, setChannelConnectionMode: () => {},
  };
}
