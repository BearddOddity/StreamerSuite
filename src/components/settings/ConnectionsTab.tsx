...
const modeInfo: Record<ConnectionMode, { label: string; desc: string }> = {
  api: { label: "API Mode", desc: "Uses platform REST APIs. More stable, no raw WebSocket." },
  ws: { label: "WebSocket Mode", desc: "Direct WebSocket connection. Needed for real-time chat." },
};
      ...
      <div className="space-y-4">
        ...
        <div className="surface-card rounded-2xl p-6 mb-5">
          <div className="section-head mb-4">
            ...
            <div className="section-head-text">
              <h4 className="section-head-title">Supported Platforms</h4>
              <p className="section-head-desc">Connect your streaming platforms</p>
            </div>
          </div>
              ...
              <div key={id} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] text-white/30">
                    {id === "twitch" && "IRC WebSocket chat + Helix API"}
                    {id === "kick" && "REST API + Pusher WebSocket"}
                    {id === "joystick" && "WebSocket (no public API)"}
                  </div>
                </div>
                <span className="badge badge-ghost text-[9px]">Not connected</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-[11px] text-white/25 leading-relaxed">
            No channels configured yet.<br />Add channels to configure connection modes.
          </p>
        </div>
      </div>
    );
        ...
        return (
          <div key={`${ch.platform}:${ch.channelId}`} className="surface-card rounded-2xl p-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => onSetConnectionMode(ch.platform, ch.channelId, "api")}
              ...
              <button
                onClick={() => onSetConnectionMode(ch.platform, ch.channelId, "ws")}
                className={`flex-1 py-2 px-3 rounded-xl text-xs font-medium border transition-all ${
            ...
            {ch.platform === "twitch" && (
              <p className="text-[10px] text-[#9146ff]/40 italic mt-2">
                ℹ️ Twitch uses IRC WebSocket for real-time chat (no public read API).
              </p>
            )}
