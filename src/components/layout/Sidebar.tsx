...
const platformMeta: Record<string, { label: string; dot: string; tag: string }> = {
  twitch: {
    label: "Twitch",
    dot: "bg-[#9146ff]",
    tag: "bg-[#9146ff]/15 text-[#9146ff] border-[#9146ff]/20",
  },
  kick: {
    label: "Kick",
    dot: "bg-[#53fc18]",
    tag: "bg-[#53fc18]/15 text-[#53fc18] border-[#53fc18]/20",
  },
  joystick: {
    label: "JoystickTV",
    dot: "bg-[#ff6b35]",
    tag: "bg-[#ff6b35]/15 text-[#ff6b35] border-[#ff6b35]/20",
  },
};
      ...
      <div className="px-4 pt-5 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center">
            <span className="text-[var(--accent-system)] text-sm">⚡</span>
          </div>
        ...
        >
          ...
          <span
            className={`ml-auto text-[10px] px-2 py-0.5 rounded-full font-semibold ${
              isMultiChat
                ? "bg-[var(--accent-system)]/15 text-[var(--accent-system)]"
      ...
      <div className="flex-1 overflow-y-auto px-3 py-1">
        <div className="flex items-center justify-between px-1 mb-2">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] text-[var(--accent-system)]/70 hover:text-[var(--accent-system)] transition-colors font-medium"
          >
          ...
          <div className="space-y-0.5">
            {channels.map((ch) => {
              ...
              const meta = platformMeta[ch.platform] ?? {
                label: ch.platform,
                dot: "bg-white/30",
                tag: "bg-white/5 text-white/30 border-white/10",
              };
              return (
                <div
                  key={key}
                ...
                >
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 ${
                      ch.isConnected ? meta.dot : "bg-white/15"
                    } ${ch.isConnected ? "status-dot-pulse" : ""}`}
                  />
                  <div className="flex-1 min-w-0">
                    ...
                    <span
                      className={`inline-block text-[9px] px-1.5 py-0.5 rounded-md border mt-1 font-semibold uppercase tracking-wider ${meta.tag}`}
                    >
                      {meta.label}
                    </span>
                  </div>
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 w-5 h-5 flex items-center justify-center rounded-md text-red-400/40 hover:text-red-400/80 hover:bg-red-500/10 text-xs transition-all"
                    title="Disconnect"
                  >
                    ✕
                  </button>
      ...
      <div className="px-4 py-3 border-t border-white/[0.04]">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-[11px] text-white/30 bg-white/[0.03] border border-white/[0.06] hover:text-white/60 hover:bg-white/[0.06] hover:border-white/[0.1] transition-all"
          >
