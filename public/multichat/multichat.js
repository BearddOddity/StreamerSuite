...
// Linktree's mark — a trunk with two branching node pairs, in the brand
// green, sized to read clearly at fallback-avatar size (44px).
const LINKTREE_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#43e660" stroke-width="2" stroke-linecap="round"><line x1="12" y1="4" x2="12" y2="20"></line><line x1="12" y1="9" x2="6" y2="13"></line><line x1="12" y1="9" x2="18" y2="13"></line><line x1="12" y1="14" x2="7" y2="18"></line><line x1="12" y1="14" x2="17" y2="18"></line><circle cx="12" cy="4" r="1.6" fill="#43e660" stroke="none"></circle><circle cx="6" cy="13" r="1.6" fill="#43e660" stroke="none"></circle><circle cx="18" cy="13" r="1.6" fill="#43e660" stroke="none"></circle><circle cx="7" cy="18" r="1.6" fill="#43e660" stroke="none"></circle><circle cx="17" cy="18" r="1.6" fill="#43e660" stroke="none"></circle></svg>`;
// Brand-colored icons for every other statically-chipped link preview
// (no public unauthenticated API to pull a real thumbnail from, so these
// replace the plain letter fallback with a recognizable mark instead).
const PRIME_GAMING_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#00A8E1"><rect x="3" y="9" width="18" height="9" rx="4.5"></rect><rect x="6.6" y="12.2" width="2.8" height="1" fill="#0b1520"></rect><rect x="7.6" y="11.2" width="1" height="2.8" fill="#0b1520"></rect><circle cx="16" cy="12.7" r="1" fill="#0b1520"></circle><circle cx="18" cy="14.7" r="1" fill="#0b1520"></circle></svg>`;
const X_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#fff"><path d="M4 4l7.5 8.2L4.3 20h2.3l6.1-6.7 4.9 6.7H21l-7.8-8.6L20 4h-2.3l-5.7 6.2L7 4H4z"></path></svg>`;
const KOFI_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#FF5E5B" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 8h11v6a4 4 0 0 1-4 4H9a4 4 0 0 1-4-4V8z"></path><path d="M16 9h1.5a2.5 2.5 0 0 1 0 5H16"></path><path d="M9 5.5c0-1 .8-1 .8-2M12 5.5c0-1 .8-1 .8-2"></path></svg>`;
const FOURTHWALL_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#7C5CFC" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8h12l-1 12H7L6 8z"></path><path d="M9 8V6a3 3 0 0 1 6 0v2"></path></svg>`;
const STREAMLABS_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#9146FF"><path d="M12 3c3 4 6 7.5 6 11a6 6 0 0 1-12 0c0-3.5 3-7 6-11z"></path></svg>`;
const STREAMELEMENTS_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#3EE07C"><path d="M13 2 4 14h6l-1 8 9-12h-6l1-8z"></path></svg>`;
const PATREON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#FF424D"><circle cx="15" cy="9" r="6"></circle><rect x="4" y="3" width="3" height="18"></rect></svg>`;
...
const INSTAGRAM_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"><defs><linearGradient id="ig-grad" x1="0" y1="24" x2="24" y2="0"><stop offset="0" stop-color="#FEDA75"></stop><stop offset="0.5" stop-color="#D62976"></stop><stop offset="1" stop-color="#4F5BD5"></stop></linearGradient></defs><rect x="3" y="3" width="18" height="18" rx="5" stroke="url(#ig-grad)" stroke-width="1.8"></rect><circle cx="12" cy="12" r="4" stroke="url(#ig-grad)" stroke-width="1.8"></circle><circle cx="17.3" cy="6.7" r="1.1" fill="url(#ig-grad)"></circle></svg>`;
...
// Best-effort real favicon for whatever domain a link points at — works for
// any site, not just the ones we've hand-built a brand icon for. Google's
// favicon service always answers (falling back to a generic globe glyph for
// domains with no real one), so there's no network round trip to gate on —
// swap it in optimistically and let onerror/the brand SVG cover failures.
function faviconUrl(url) {
  try { return `https://www.google.com/s2/favicons?sz=64&domain=${encodeURIComponent(new URL(url).hostname)}`; }
  catch { return null; }
}
...
function linkPreviewStaticChip(kind, tag, title, sub, url, fallbackLetter, iconSvg) {
  ...
  const showBrandFallback = () => { if (iconSvg) icon.innerHTML = iconSvg; else icon.textContent = fallbackLetter; }; // trusted constant, never user-supplied
  const fav = faviconUrl(url);
  if (fav) {
    const img = el("img", "cv-linkpreview-favicon");
    img.src = fav; img.alt = ""; img.loading = "lazy";
    img.onerror = () => { img.remove(); showBrandFallback(); };
    icon.append(img);
  } else {
    showBrandFallback();
  }
