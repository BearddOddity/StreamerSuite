...
function printPage(p: (typeof TWITCH_DOC_PAGES)[number]) {
  console.log(`${p.section.padEnd(18)} ${p.title.padEnd(40)} ${p.url}`);
}

async function list() {
  console.log(`Twitch developer docs — ${TWITCH_DOC_PAGES.length} pages\n`);
  let lastSection = "";
  for (const p of TWITCH_DOC_PAGES) {
    ...
    console.log(`  - ${p.title.padEnd(40)} ${p.url}`);
  }
}
...
async function search(term: string, docsDir = DEFAULT_DOCS_DIR) {
  ...
  if (results.length === 0) {
    console.log(`No Twitch docs pages matched "${term}".`);
    console.log(`Tip: run "npm run docs:twitch" first to fetch page bodies for full-text search.`);
    return;
  }
